...
def get_vectorstore_retriever(*, limit: int = 3, embedding_device: str = None):
    return get_vectorstore(embedding_device=embedding_device).as_retriever(
        search_kwargs={"k": limit}
    )


def get_retriever(*, rerank: bool = True, limit: int = 3, embedding_device: str = None):
    if rerank:
        return ContextualCompressionRetriever(
            ...
            base_retriever=get_vectorstore_retriever(
                limit=limit * 3, embedding_device=embedding_device
            ),
        )
    else:
